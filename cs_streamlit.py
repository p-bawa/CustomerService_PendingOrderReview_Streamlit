...
#     st.title("🎯 Sales Order Review")
#     st.subheader("✨ Overview")
#     st.write("""
#     Easily manage Submitter IDs for each order, ensuring precise tracking. 
#     Our dashboard offers an efficient way to review order accuracy!
#     """)
#     dataset = session.table(TABLE_NAME)
#     df = pd.DataFrame(dataset.collect())
#     # df[COL_ORDERED_DATE] = pd.to_datetime(df[COL_ORDERED_DATE]) 
#     # df['HOLD_APPLIED_DATE'] = pd.to_datetime(df['HOLD_APPLIED_DATE'])
#     # df = df[['ID', 'ORDERED_DATE','SALES_ORDER_NUMBER', 'SUBMITTER_EMAIL_ID', 'NEW_SUBMITTER_EMAIL_ID', 'REVIEWER_EMAIL_ID', 'SOURCE_ORDER_NUMBER', 'SALES_ORDER_TYPE']]
#     ##########
#     # Filters
#     ##########
...
#             value=ten_days_ago,  # Default to 10 days ago
#             format='MM/DD/YYYY'
#         )
...
#     #         value=ten_days_ago,  # Default to today
#     #         format='MM/DD/YYYY'
#     #     )    
#     # with col10:
...
#     df = df[
#         (df[COL_ORDERED_DATE] >= pd.to_datetime(start_date)) & 
#         (df[COL_ORDERED_DATE] <= pd.to_datetime(end_date))
#     ]
...
#     with st.form("Edit Submitter and Reviewer Email IDs"):
#         st.caption("Edit Submitter and Reviewer Email IDs")
...
#             # Ensure the edited DataFrame is converted to a Snowpark DataFrame
